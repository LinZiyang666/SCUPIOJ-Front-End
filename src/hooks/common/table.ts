...
import { levelMap } from '@/service/request';
...
function useTableColumn<TableData extends BaseData, CustomColumnKey = never>(
  ...
  const columns: any = computed(() => getColumns());
  ...
  function getFilteredColumns(aColumns: TableColumn<TableData, CustomColumnKey>[]) {
    ...
    aColumns.forEach((column: any) => {
      if (!column.p || (column.p && levelMap[column.p].includes(userInfo.roles.join(',')))) {
    ...
    const cols = filteredColumns.value
      ...
      .map((column: any) => {
        if (column.key === SELECTION_KEY) {
          return allColumns.value.find((col: any) => col.type === 'selection');
        }
        return allColumns.value.find((col: any) => (col as DataTableBaseColumn).key === column.key);
      });
